...
class QuotationController extends Controller
{
    ...
    public function downloadImage()
    {
        ...
        $y += 28;
        imagestring($img, $font, 20, $y, 'Country & Service: ' . ($result['country'] ?? '') . ' - ' . ($result['service'] ?? ''), $gray);
        $y += 24;
        imagestring($img, $font, 20, $y, 'Quantity (kg): ' . ($result['qty'] ?? ''), $gray);
        $y += 28;
        $priceLine = !empty($result['dealer_applied'])
            ? 'Dealer total: ' . number_format($result['dealer_unit_price'] ?? 0, 2) . ' x ' . ($result['qty'] ?? '') . ' kg = ' . number_format($result['total_dealer'] ?? 0, 2)
            : 'Total: ' . number_format($result['customer_unit_price'] ?? 0, 2) . ' x ' . ($result['qty'] ?? '') . ' kg = ' . number_format($result['total_customer'] ?? 0, 2);
        imagestring($img, $font, 20, $y, $priceLine, $text);
        $y += 24;
        $y += 16;
    ...
    public function downloadText()
    {
        ...
        $lines = [
            'QUOTATION',
            '================',
            'Country & Service: ' . ($result['country'] ?? '') . ' - ' . ($result['service'] ?? ''),
            'Quantity (kg): ' . ($result['qty'] ?? ''),
            '',
            $priceLine,
        ];
        $lines[] = '';
