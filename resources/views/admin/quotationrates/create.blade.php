...
@section('title', 'Add Quotation Rate - Admin')
...
@section('content')
  ...
  <div style="display:flex; gap:8px; align-items:center; margin-bottom:12px">
    ...
    <h2 style="margin:0">Add Quotation Rate</h2>
  </div>

  <form method="POST" action="{{ route('admin.quotationrates.store') }}">
    @csrf
    <label for="country">Country</label>
    <input id="country" type="text" name="country" value="{{ old('country') }}" required placeholder="e.g. USA, India">

    <label for="service">Service</label>
    <input id="service" type="text" name="service" value="{{ old('service') }}" required placeholder="e.g. DHL, FedEx, UPS">

    <label for="customer_price">Customer price (unit)</label>
    <input id="customer_price" type="number" step="0.01" min="0" name="customer_price" value="{{ old('customer_price', '0') }}" required>

    <label for="dealer_price">Dealer price (unit)</label>
    <input id="dealer_price" type="number" step="0.01" min="0" name="dealer_price" value="{{ old('dealer_price', '0') }}" required>
    ...
    <div style="display:flex; flex-wrap:wrap; gap:12px; padding:12px; background:var(--accent); border-radius:10px; border:1px solid var(--border)">
      @forelse(($dealers ?? []) as $d)
        <label style="display:flex; align-items:center; gap:6px; cursor:pointer; white-space:nowrap">
          <input type="checkbox" name="dealer_ids[]" value="{{ $d->id }}" {{ in_array($d->id, old('dealer_ids', [])) ? 'checked' : '' }}>
