  ...
  <style>
    ...
    * { box-sizing: border-box }
    ...
    @media (max-width: 480px) {
      ...
      .hero-content { padding: 0 12px; width: 100%; }
        ...
        @if(isset($navLinks) && $navLinks->count())
          @foreach($navLinks as $nl)
            ...
            @php
              ...
              $homePath = trim((string) parse_url(url('/'), PHP_URL_PATH), '/');
              $isHomeUrl = ($path === $homePath || $path === 'home');
            @endphp
            <a href="{{ $linkUrl }}" @if($isHomeUrl) class="js-home-link" @endif @if($nl->target) target="{{ $nl->target }}" rel="noopener" @endif>
